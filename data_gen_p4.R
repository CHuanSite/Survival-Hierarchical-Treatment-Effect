...
beta.gen <- function(p = 10, m = 5, l = 3, seed = 1120, beta.trt = 1){
  set.seed(seed)
  ...
  for(i in 1 : length(gamma)){
      gamma[i] = runif(1, -abs(beta.main[i]), abs(beta.main[i]))
  }
  if(sum(abs(gamma)) > beta.trt){
      gamma = gamma / sum(abs(gamma)) * beta.trt
  }
...
gen.data <- function(n = 300, seed = 1032, p = 10, m = 5, 
                     ...
                     sigma = 1) {
  ####Generate betas#####
  beta.dat <- beta.gen(p = p, m = m, l = l, beta.trt = beta.trt)
