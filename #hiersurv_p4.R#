...
# source("./new_work/Simulations/data_gen_p4.R")
source("/users/hchen/new_work/Simulations/data_gen_p4.R")
library(BB)
library(glmnet)
...
comp.model <- function(n = 10500, n1 = 500, p = 50, m = 25, seed = 1031, 
  ...
  for(i in c(2,4,5)) {
    ...
    end_time <- Sys.time()
    comp.time <- c(comp.time, end_time - start_end)
...
save(comp.result, file = paste("/users/hchen/new_work/Simulations/p4_output/", "output", k, ".RData", sep = ""))
