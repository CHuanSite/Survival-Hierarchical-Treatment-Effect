...
beta.gen <- function(p = 10, m = 5, l = 3, seed = 1120){
  ...
  # inter.effect.index <- sort(sample(which(!(c(1 : p) %in% main.effect.index)), l))
  inter.effect.index <- sort(sample(1 : p, l))
  ...
  gamma <- rep(0, p)
  gamma[inter.effect.index] <- rnorm(length(inter.effect.index), 0, 1)
  # gamma[inter.effect.index] <- runif(length(inter.effect.index), -1, 1)
...
gen.data <- function(n = 300, seed = 1032, p = 10, m = 5, 
  ...
  ####Generate y's
  y.mean <- beta.intercept + A * beta.trt + vars.dat %*% beta.main + (A * vars.dat) %*% beta.inter + var.dat.inter %*% eta.inter
