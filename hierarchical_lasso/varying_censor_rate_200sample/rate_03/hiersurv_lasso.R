...
# source("./new_work/Simulations/data_gen_lasso.R")
source("/users/hchen/new_work/Simulations/hierarchical_lasso/varying_censor_rate_200sample/rate_03/data_gen_lasso.R")
library(BB)
library(glmnet)
...
comp.result <- comp.model(n = 10500, n1 = 200, p = 50, seed = k,
                          m = 25, l = 5, beta.trt = 1, beta.intercept = 0.5,
                          sigma = 1, cv.method = "concordance")
save(comp.result, file = paste("/users/hchen/new_work/Simulations/hierarchical_lasso/varying_censor_rate_200sample/rate_03/out_lasso/", "output", k, ".RData", sep = ""))
